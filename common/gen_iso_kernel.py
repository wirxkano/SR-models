...
def generate_mod_LR_bic():
    ...
    savedir = f"/root/quoc-huy/materials/{datasetname}"
    ...
    pca_matrix = torch.load(
        "/root/quoc-huy/DAN/pca_matrix/DANv1/pca_matrix.pth", map_location=lambda storage, loc: storage
    )
    ...
    for i in range(num_files):
        ...
        for sig in np.linspace(1.6, 1.6, 1):
            ...
            kernel = util.tensor2img(ker_map)
            cv2.imwrite(os.path.join(saveLRblurpath, filename), image_LR_blur)
            cv2.imwrite(os.path.join(saveLRblurpath, filename, '_kernel'), kernel)
