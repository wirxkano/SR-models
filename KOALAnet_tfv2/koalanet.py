...
class KOALAnet:
	...
	def upsampling_network_baseline(self, input_LR, factor, kernel, channels=3, reuse=False, scope='SISR_DUF'):
		with tf.variable_scope(scope, reuse=reuse):
			ch = 64
			n_res = 12
			net = conv2d(input_LR, ch, 3, scope='conv2d')
			for res in range(n_res):
				net = res_block(net, ch, 3, scope='Residual_block0_' + str(res + 1))
			net = tf.nn.relu(net)
			# upsampling kernel branch
			k2d = tf.nn.relu(conv2d(net, ch * 2, 3, scope='conv2d_k2d_1'))
			k2d = conv2d(k2d, kernel * kernel * factor * factor, 3, scope='conv2d_k2d_2')
			# rgb residual image branch
			rgb = tf.nn.relu(conv2d(net, ch * 2, 3, scope='conv2d_rgb_1'))
			rgb = tf.depth_to_space(rgb, 2)
			if factor == 4:
				rgb = tf.nn.relu(conv2d(rgb, ch, 3, scope='conv2d_rgb_2'))
				rgb = tf.depth_to_space(rgb, 2)
			rgb = conv2d(rgb, channels, 3, scope='conv2d_rgb_3')
	...
	def upsampling_network(self, input_LR, k2d_ds, factor, kernel, channels=3, reuse=False, scope='SISR_DUF'):
		with tf.variable_scope(scope, reuse=reuse):
			...
			net = tf.nn.relu(net)
			# upsampling kernel branch
			k2d = tf.nn.relu(conv2d(net, ch * 2, 3, scope='conv2d_1'))
			k2d = conv2d(k2d, kernel * kernel * factor * factor, 3, scope='conv2d_2')
			# rgb residual image branch
			rgb = tf.nn.relu(conv2d(net, ch * 2, 3, scope='conv2d_3'))
			rgb = tf.depth_to_space(rgb, 2)
			if factor == 4:
				rgb = tf.nn.relu(conv2d(rgb, ch, 3, scope='conv2d_4'))
				rgb = tf.depth_to_space(rgb, 2)
			rgb = conv2d(rgb, channels, 3, scope='conv2d_5')
			# local filtering and upsampling
			output_k2d = local_conv_us(input_LR, k2d, factor, channels, kernel)
			output = output_k2d + rgb
	...
	def downsampling_network(self, input_LR, kernel, reuse=False, scope='SISR'):
		with tf.variable_scope(scope, reuse=reuse):
			...
			n = tf.nn.relu(conv2d(n, ch, 3, scope='conv2d'))
			k2d = conv2d(n, kernel * kernel, 3, scope='conv2d_1')
		return k2d
	...
	def build_model(self, args):
		...
		if self.phase == 'train':
			...
			""" Visualization """
			self.k2d_ds_test = kernel_normalize(self.k2d_ds_test, self.down_kernel)
			k2d_mean = tf.reduce_mean(self.k2d_ds_test, axis=[1, 2], keepdims=True)
			self.k2d_ds_vis = tf.reshape(k2d_mean, [1, self.down_kernel, self.down_kernel, 1])  # [B, down_kernel, down_kernel, 1]
			kernel_min = tf.reduce_min(self.k2d_ds_vis, axis=(1, 2), keepdims=True)
			kernel_max = tf.reduce_max(self.k2d_ds_vis, axis=(1, 2), keepdims=True)
			self.scale_vis = (self.patch_size*self.factor)//self.bicubic_size
			self.k2d_ds_vis = local_conv_vis_ds(self.k2d_ds_vis, kernel_min, kernel_max, 3, self.scale_vis)
	...
	def measure_glops(self):
		import time
		INPUT_HEIGHT = 256
		INPUT_WIDTH = 256
		INPUT_CHANNELS = 3
		test_lr_p = np.random.randn(1, INPUT_HEIGHT, INPUT_WIDTH, INPUT_CHANNELS).astype(np.float32)
		...
		_ = self.sess.run(
			self.output_test,
			feed_dict={self.test_lr: test_lr_p},
			options=run_options,
		...
		# warm-up
		for i in range(5):
			self.sess.run(self.output_test, feed_dict={self.test_lr: test_lr_p})
	...
	def test(self):
		...
		lpips_metric = pyiqa.create_metric('lpips', device='cuda' if torch.cuda.is_available() else 'cpu')
		niqe_metric = pyiqa.create_metric('niqe', device='cuda' if torch.cuda.is_available() else 'cpu')
		...
		for test_cnt in range(len(self.list_test_lr)):
			...
			for p in range(self.test_patch[0] * self.test_patch[1]):
				...
				st = time.time()
				output_test_p, k2d_vis_p = self.sess.run(
					[self.output_test, self.k2d_ds_vis],
					feed_dict={self.test_lr: test_lr_p}
				)
			...
			if self.eval:
				sr_img = np.squeeze(output_test)
				hr_img = np.squeeze(test_hr)
    
				# sr_img = sr_img.clip(0, 1)
				# hr_img = hr_img.clip(0, 1)
				
				if c == 3:  # RGB image
					sr_y = rgb2ycbcr_lib(sr_img)[:, :, 0]
					hr_y = rgb2ycbcr_lib(hr_img)[:, :, 0]
					# print('>>>', sr_y.max(), hr_y.min())
					data_range = 255.0
				else:  # Grayscale
					sr_y = sr_img * 255.0 if sr_img.max() <= 1 else sr_img
					hr_y = hr_img * 255.0 if hr_img.max() <= 1 else hr_img
					data_range = 255.0
				
				test_PSNR = peak_signal_noise_ratio(hr_y, sr_y, data_range=data_range)
				test_SSIM = structural_similarity(hr_y, sr_y, data_range=data_range)
        
				sr_rgb = sr_img if sr_img.max() <= 1 else sr_img / 255.0
				hr_rgb = hr_img if hr_img.max() <= 1 else hr_img / 255.0
    
				sr_tensor = torch.from_numpy(sr_rgb).permute(2, 0, 1).unsqueeze(0).float()
				hr_tensor = torch.from_numpy(hr_rgb).permute(2, 0, 1).unsqueeze(0).float()
    
				sr_tensor = sr_tensor.clamp(0, 1)
				hr_tensor = hr_tensor.clamp(0, 1)
				
				test_LPIPS = lpips_metric(sr_tensor, hr_tensor).item()
				test_NIQE = niqe_metric(sr_tensor).item()
				
				avg_test_PSNR += test_PSNR
				avg_test_SSIM += test_SSIM
				avg_test_LPIPS += test_LPIPS
				avg_test_NIQE += test_NIQE
				
				print(" <Test> [%4d/%4d]-th images, time: %4.4f(s), PSNR: %2.2f[dB], SSIM: %.4f, LPIPS: %.4f, NIQE: %.4f"
              			% (int(test_cnt+1), len(self.list_test_lr), inf_time, test_PSNR, test_SSIM, test_LPIPS, test_NIQE))
			else:
				print(" <Test> [%4d/%4d]-th images, time: %4.4f(seconds)  "
					  % (int(test_cnt + 1), len(self.list_test_lr), inf_time))
			# save predicted SR images
			save_dir = self.save_dir
			save_path = os.path.join(save_dir, 'imgs', os.path.basename(self.list_test_lr[test_cnt]))
			save_ker_path = os.path.join(save_dir, 'kernels', os.path.basename(self.list_test_lr[test_cnt]))
			if not os.path.exists(save_dir):
				os.makedirs(save_dir, exist_ok=True)
			save_img(k2d_vis_p, save_ker_path)
			save_img(output_test, save_path)

		if self.eval:
			print(avg_test_PSNR, avg_test_SSIM, avg_test_LPIPS, avg_test_NIQE)
			avg_test_PSNR /= float(len(self.list_test_lr))
			avg_test_SSIM /= float(len(self.list_test_lr))
			avg_test_LPIPS /= float(len(self.list_test_lr))
			avg_test_NIQE /= float(len(self.list_test_lr))
			...
			print("######### Average Test LPIPS (RGB): %.4f  #########" % avg_test_LPIPS)
			print("######### Average Test FID (RGB): %.4f  #########" % avg_test_FID)
			print("######### Average Test NIQE (RGB): %.4f  #########" % avg_test_NIQE)
