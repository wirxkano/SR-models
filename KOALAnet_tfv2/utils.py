...
def compute_ssim(img_gt, img_out):
    """Compute SSIM between two images."""
    return ssim(img_gt, img_out, data_range=255)


def compute_y_ssim(img_gt_rgb, img_out_rgb):
    """
    Compute SSIM on Y channel (luminance) of YCbCr color space.
    
    Args:
        img_gt_rgb: Ground truth RGB image in range [-1, 1]
        img_out_rgb: Output RGB image in range [-1, 1]
    
    Returns:
        ssim_value: SSIM score between 0 and 1
    """
    # images must be in range [-1, 1] float or double
    img_gt_rgb = np.squeeze(img_gt_rgb)
    img_out_rgb = np.squeeze(img_out_rgb)
    ...
    ssim_value = compute_ssim(img_gt_yuv, img_out_yuv)
    return ssim_value
