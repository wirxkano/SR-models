...
class GIASpatial(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        ## new code
        _, _, H, W = feat1.shape
        pad_w = min(mod_pad_w, W - 1)
        pad_h = min(mod_pad_h, H - 1)
        feat1 = F.pad(feat1, (0, pad_w, 0, pad_h), 'reflect')
        feat2 = F.pad(feat2, (0, pad_w, 0, pad_h), 'reflect')
        # to windows
        B, C, H, W = feat1.size()
        ...
        if H < win_h or W < win_w:
            feat1 = feat1[:, :, :h_ori, :w_ori]
            feat2 = feat2[:, :, :h_ori, :w_ori]
            return feat1, feat2
