...
class options:
    ...
    def get_config(self, img_name):
        ...
        clean_name = re.sub(r'[_-]?x\d+[a-zA-Z]*$', '', name)

        self.conf.abs_img_name = clean_name
        self.conf.input_image_path = os.path.join(self.conf.input_dir, img_name)
        self.conf.gt_path = os.path.join(self.conf.gt_dir, clean_name + ext) if self.conf.gt_dir != '' else None
