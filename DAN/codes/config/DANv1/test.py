...
#     dataset_dir = os.path.join("/root/quoc-huy/all-tested-results/DAN", test_set_name)
    ...
    for test_data in test_loader:
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                logger.warning(f"Skipping {img_name} due to casting error: {str(e)}")
                continue
        ...
        ### Kernel
        blur_kernel = util.reconstruct_from_pca(visuals["ker"])
        ...
        if need_GT:
            ...
            if gt_img.shape[2] == 3:  # RGB image
                ...
                psnr_y = peak_signal_noise_ratio(gt_img_y, sr_img_y, data_range=255.0)
                ssim_y = structural_similarity(gt_img_y, sr_img_y, data_range=255.0)
                ...
                logger.info(
                    "img:{:15s} - PSNR_Y: {:.6f} dB; SSIM_Y: {:.6f}; LPIPS: {:.6f}; NIQE: {:.6f}.".format(
                        img_name, psnr_y, ssim_y, lpips_val.item(), niqe_val.item()
                    )
                )
            else:
                # Grayscale image
                psnr_y = peak_signal_noise_ratio(cropped_gt_img, cropped_sr_img, data_range=255.0)
                ssim_y = structural_similarity(cropped_gt_img, cropped_sr_img, data_range=255.0)
                
                test_results["psnr_y"].append(psnr_y)
                test_results["ssim_y"].append(ssim_y)
                
                # For grayscale, still calculate NIQE but skip LPIPS and FID
                sr_tensor = torch.from_numpy(cropped_sr_img).unsqueeze(0).unsqueeze(0).float().to(device)
                niqe_val = niqe_metric(sr_tensor)
                test_results["niqe"].append(niqe_val.item())
                
                logger.info(
                    "img:{:15s} - PSNR_Y: {:.6f} dB; SSIM_Y: {:.6f}; NIQE: {:.6f}.".format(
                        img_name, psnr_y, ssim_y, niqe_val.item()
                    )
                )
        else:
            logger.info(img_name)
    ...
    if test_results["psnr_y"] and test_results["ssim_y"]:
        ...
        logger.info(
            "----Average results for {}----\n\tPSNR_Y: {:.2f} dB; SSIM_Y: {:.4f}\n".format(
                test_set_name, ave_psnr_y, ave_ssim_y
            )
        )
        
        if test_results["lpips"]:
            ave_lpips = sum(test_results["lpips"]) / len(test_results["lpips"])
            logger.info("\tLPIPS: {:.4f}".format(ave_lpips))
            
        if fid_val > 0:
            logger.info("\tFID: {:.4f}\n".format(fid_val))
        
        if test_results["niqe"]:
            ave_niqe = sum(test_results["niqe"]) / len(test_results["niqe"])
            logger.info("\tNIQE: {:.4f}".format(ave_niqe))
