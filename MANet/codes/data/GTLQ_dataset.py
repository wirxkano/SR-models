...
class GTLQDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if self.LR_paths and self.GT_paths:
            print("--------------LENGTH", len(self.LR_paths))
            print("--------------LENGTH", len(self.GT_paths))
    ...
    def __getitem__(self, index):
        ...
        if not (GT_path.lower().endswith(valid_exts) and LR_path.lower().endswith(valid_exts)):
            next_index = (index + 1) % len(self.GT_paths)
