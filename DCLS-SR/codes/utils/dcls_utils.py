...
def normkernel_to_downkernel(rescaled_blur_hr, rescaled_hr, ksize, eps=1e-10):
    ...
    blur_img = torch.view_as_real(blur_img)
...
def deconv(inv_ker_f, fft_input_blur):
    ...
    deblur = torch.fft.ifftn(torch.view_as_complex(deblur_f), dim=(-3, -2, -1)).real
    # deblur = torch.fft.irfft(deblur_f, 3, onesided=False)
    return deblur
...
def convert_otf2psf(otf, size):
    ...
    psf = torch.fft.ifftn(torch.view_as_complex(otf), dim=(-3, -2, -1)).real
    
    # psf = torch.fft.irfft(otf, 3, onesided=False)
